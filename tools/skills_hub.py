...
# ---------------------------------------------------------------------------
# Nazar centralized index source
# ---------------------------------------------------------------------------

NAZAR_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
NAZAR_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "nazar-index.json"
NAZAR_INDEX_TTL = 6 * 3600  # 6 hours
...
def _load_hermes_index() -> Optional[dict]:
    ...
    We cache it locally for NAZAR_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if NAZAR_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - NAZAR_INDEX_CACHE_FILE.stat().st_mtime
            if age < NAZAR_INDEX_TTL:
                return json.loads(NAZAR_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(NAZAR_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
    ...
    # Cache locally
    try:
        NAZAR_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        NAZAR_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
