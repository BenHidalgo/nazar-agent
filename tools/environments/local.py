...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from nazar_cli.config import OPTIONAL_ENV_VARS
...
def _find_bash() -> str:
    ...
    _hermes_portable_git = os.path.join(_local_appdata, "nazar", "git") if _local_appdata else ""
    ...
    raise RuntimeError(
        "Git Bash not found. Nazar Agent requires Git for Windows on Windows.\n"
...
def _make_run_env(env: dict) -> dict:
    ...
    # npm …) into {NAZAR_HOME}/home/ when that directory exists.  Only the
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``NAZAR_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under NAZAR_HOME.  Using
