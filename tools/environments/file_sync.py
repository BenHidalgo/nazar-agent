...
def iter_sync_files(container_base: str = "/root/.nazzar") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.nazzar to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        lock_path = (hermes_home or get_nazar_home()) / ".sync.lock"
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.nazzar/skills/a.md`` →
        ``~/.hermes/skills/a.md``, a new remote file at
        ``/root/.nazzar/skills/b.md`` maps to ``~/.hermes/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
