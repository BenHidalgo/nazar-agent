...
# Each tuple is (new_subpath, old_name) matching hermes_constants.get_nazar_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_nazar_dir()`` for backward compatibility with old directory layouts.
    """
    from nazar_constants import get_nazar_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_nazar_dir(new_subpath, old_name)
        if host_dir.is_dir():
