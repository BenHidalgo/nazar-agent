...
Backend is selected during ``nazar tools`` setup (web.backend in config.yaml).
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nazar tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `nazar tools` repaint.
        try:
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "nousresearch.com" or host.endswith(".nousresearch.com") or host == "nazar-agent.local"
