...
The PID file lives at ``{NAZAR_HOME}/gateway.pid``.  NAZAR_HOME defaults to
``~/.hermes`` but can be overridden via the environment variable.  This means
separate NAZAR_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting NAZAR_HOME."""
    home = get_nazar_home()
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "nazar" / _LOCKS_DIRNAME
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different NAZAR_HOME dirs).
    """
