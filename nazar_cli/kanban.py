...
from nazar_cli import kanban_db as kb
from nazar_cli import kanban_swarm as ks
from nazar_cli.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this NAZAR_HOME and its
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            ...
            "See https://nazar-agent.local/docs/user-guide/features/kanban "
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # NAZAR_HOME. Previously only `init` and `daemon` triggered
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    DB, so a fresh NAZAR_HOME that has never called ``kanban init``
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from nazar_cli import kanban_diagnostics as kd
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from nazar_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from nazar_cli import kanban_decompose as decomp
