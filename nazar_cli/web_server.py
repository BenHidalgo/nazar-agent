...
app = FastAPI(title="Nazar Agent", version=__version__)
...
_ACTION_LOG_DIR: Path = get_nazar_home() / "logs"
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://portal.nazar-agent.local",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from nazar_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from nazar_cli.auth import clear_provider_auth
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from nazar_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from nazar_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_nazar_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from nazar_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from nazar_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, NAZAR_HOME)."""
    from nazar_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process NAZAR_HOME at import time. The dashboard is a single
...
async def get_skills():
    ...
    from nazar_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from nazar_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from nazar_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    """
    from nazar_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_nazar_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    from nazar_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_nazar_home() / "plugins", "user"),
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from nazar_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_nazar_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from nazar_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from nazar_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nazar_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nazar_cli.plugins_cmd import dashboard_remove_user_plugin
