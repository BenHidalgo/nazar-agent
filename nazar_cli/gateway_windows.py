...
_TASK_DESCRIPTION = "Nazar Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    ...
    from nazar_cli.gateway import _profile_arg
...
def get_task_name() -> str:
    ...
    from nazar_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<NAZAR_HOME>/gateway-service/<task_name>.cmd`` so per-profile
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports NAZAR_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    lines.append(f'set "NAZAR_HOME={hermes_home}"')
...
def _prompt_install_choices(
    ...
    from nazar_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from nazar_cli.gateway import find_gateway_pids, _profile_arg
...
def stop() -> None:
    ...
    _assert_windows()
    from nazar_cli.gateway import kill_gateway_processes
