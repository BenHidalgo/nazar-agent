    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Nazar Agent" comment and following line
                if '# Nazar Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('nazar' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'nazar' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def uninstall_gateway_service():
    ...
    try:
        from nazar_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from nazar_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NAZAR_HOME`` and ``HERMES_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # Also match if NAZAR_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "nazar".  We don't want to catch
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete NAZAR_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NAZAR_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NAZAR_HOME directory.
    ...
    current NAZAR_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's NAZAR_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    print(color("│            ⚕ Nazar Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their NAZAR_HOME dirs. Otherwise
    ...
    if _is_windows():
        ...
        log_info("Removing NAZAR_HOME / HERMES_GIT_BASH_PATH User env vars...")
    ...
    #     under NAZAR_HOME but they're install tooling, not config — safe to
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile NAZAR_HOME dirs live under
    ...
    print()
    print("Thank you for using Nazar Agent! ⚕")
    print()
