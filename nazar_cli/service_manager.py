...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from nazar_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from nazar_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from nazar_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from nazar_cli.gateway import _probe_systemd_service_running
...
class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from nazar_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from nazar_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from nazar_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from nazar_cli.gateway import _probe_launchd_service_running
...
class WindowsServiceManager(_RegistrationUnsupportedMixin):
    ...
    def is_running(self, name: str) -> bool:
        from nazar_cli import gateway_windows
        from nazar_cli.gateway import find_gateway_pids
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources NAZAR_HOME (and any extra env) via with-contenv —
             so e.g. ``-e NAZAR_HOME=/data/hermes`` is honored at run
        ...
        root NAZAR_HOME profile" (the implicit profile that exists at
        the top of $NAZAR_HOME, not under profiles/). It must be
        ...
        would instead look up ``$NAZAR_HOME/profiles/default/`` — a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${NAZAR_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the NAZAR_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e NAZAR_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{NAZAR_HOME:=/opt/data}}"\n'
            f'log_dir="$NAZAR_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
