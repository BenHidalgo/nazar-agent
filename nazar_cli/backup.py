...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside NAZAR_HOME.
...
# Critical state files to include in quick snapshots (relative to NAZAR_HOME).
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of NAZAR_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<NAZAR_HOME>/backups/pre-update-<timestamp>.zip`` and
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NAZAR_HOME under ``backups/`` before a
    ...
    ``<NAZAR_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
