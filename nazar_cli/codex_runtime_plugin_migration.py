...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (NAZAR_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a NAZAR_HOME pointing
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    NAZAR_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # NAZAR_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_nazar_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever NAZAR_HOME its launcher sets
    ...
    # — that would override the launcher's NAZAR_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("NAZAR_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("NAZAR_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["NAZAR_HOME"] = hermes_home
