...
# $NAZAR_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NAZAR_HOME/models_dev_cache.json directly (no network) so this
...
def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://portal.nazar-agent.local").rstrip("/")
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from nazar_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.nazar-agent.local"
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from nazar_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from nazar_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from nazar_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from nazar_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from nazar_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from nazar_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from nazar_cli.auth import read_credential_pool
        from nazar_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from nazar_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
