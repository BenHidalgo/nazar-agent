"""
Interactive setup wizard for Nazar Agent.
...
_DOCS_BASE = "https://nazar-agent.local/docs"
...
from nazar_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from nazar_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from nazar_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from nazar_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        ...
        from nazar_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    try:
        from nazar_cli.auth import get_provider_auth_state
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from nazar_cli.main import select_provider_and_model
    try:
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from nazar_cli.auth_commands import auth_add_command
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    """
    try:
        from nazar_cli.auth import get_xai_oauth_auth_status
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://nazar-agent.local/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under NAZAR_HOME, and print
    ...
    """
    try:
        from nazar_cli.slack_cli import _build_full_manifest
...
def _setup_qqbot():
    ...
    from nazar_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://nazar-agent.local/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://nazar-agent.local/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from nazar_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if any_messaging:
        ...
        if service_running:
            if supports_systemd and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd:
                        ...
                        from nazar_cli import gateway_windows
            ...
            from nazar_constants import is_container
            if is_container():
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``nazar_cli.auth`` — lists every supported
    ...
    """
    try:
        from nazar_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nazar_cli.config
    ...
    if section_key == "model":
        ...
        from nazar_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    from nazar_cli.auth import get_nous_auth_status
    ...
    print()
    print_info("  Sign up: https://portal.nazar-agent.local/manage-subscription")
    print()
...
def run_setup_wizard(args):
    ...
    print(
        color(
            "│             ⚕ Nazar Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Nazar Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
