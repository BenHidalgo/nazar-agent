...
``$NAZAR_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $NAZAR_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$NAZAR_HOME``,
    ...
    ``$NAZAR_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's NAZAR_HOME (typically /opt/data).
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $NAZAR_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    hermes_home = Path(os.environ.get("NAZAR_HOME", "/opt/data"))
