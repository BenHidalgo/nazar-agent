...
deployments where ``NAZAR_HOME`` points outside ``~/.hermes`` (e.g.
``/opt/hermes``), ``<root>`` is ``NAZAR_HOME``. Legacy env-var
...
def kanban_home() -> Path:
    ...
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``NAZAR_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NAZAR_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NAZAR_HOME`` would silently fork the board per profile,
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            from nazar_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from nazar_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from nazar_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _resolve_hermes_argv() -> list[str]:
    ...
    2. ``shutil.which("nazar")`` — the console-script shim, normalized to
    ...
    hermes_bin = _safe_which_no_cwd("nazar") if _IS_WINDOWS else shutil.which("nazar")
    if hermes_bin:
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    # An unset NAZAR_HOME means the worker falls back to the default root
...
def _default_spawn(
    ...
    *,
    ...
    # Inject NAZAR_HOME so the worker reads the profile-scoped config.yaml
    ...
    try:
        env["NAZAR_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # NAZAR_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # rewrites NAZAR_HOME), its kanban paths still match the
    ...
    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped NAZAR_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("NAZAR_HOME")):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Nazar Agent or activate its venv before running the kanban dispatcher."
        )
