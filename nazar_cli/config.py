"""
Configuration management for Nazar Agent.
...
from nazar_cli.default_soul import DEFAULT_SOUL_MD
...
def is_managed() -> bool:
    ...
    or a .managed marker file in NAZAR_HOME (set by the NixOS activation
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NAZAR_HOME/.container-mode.
    ...
    exec_user = info.get("exec_user", "nazar")
...
def _secure_dir(path):
    ...
    The mode can be overridden via the NAZAR_HOME_MODE environment variable
    (e.g. NAZAR_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NAZAR_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NAZAR_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NAZAR_HOME if the user doesn't have one yet."""
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"NAZAR_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        # model; override via `nazar model` → auxiliary → Curator to route
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Nazar Agent*" header.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://nazar-agent.local/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `nazar model` invocation; network failures
    ...
    # ``nazar update`` behaviour.
    "updates": {
        # Run a full ``hermes backup``-style zip of NAZAR_HOME before every
        # ``nazar update``.  Backups land in ``<NAZAR_HOME>/backups/`` and
        # can be restored with ``hermes import <path>``.  Off by default —
        # on large NAZAR_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<NAZAR_HOME>/lsp/bin/`` on first use.
    ...
    # `nazar tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nazar model' for endpoint-specific config)",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `nazar update`.  Skill-config prompting is a
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nazar setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nazar doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NAZAR_HOME", "~/.hermes")
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``NAZAR_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`nazar tools`, `nazar setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `nazar tools`
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if missing_config:
            ...
            print("    Run 'nazar config migrate' to add them")
