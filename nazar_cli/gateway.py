    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"NAZAR_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. NAZAR_HOME
        ...
        # NAZAR_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "NAZAR_HOME=" in command and f"NAZAR_HOME={current_home}" not in command:
            return False
        return True
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from nazar_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    ...
    unit_home = env.get("NAZAR_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("NAZAR_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["NAZAR_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from nazar_cli.service_manager import detect_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from nazar_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (NAZAR_HOME-scoped).
...
from nazar_constants import is_container, is_termux, is_wsl
...
SERVICE_DESCRIPTION = "Nazar Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current NAZAR_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (NAZAR_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary NAZAR_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when NAZAR_HOME is a named profile.
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this NAZAR_HOME.

    Default ``~/.hermes`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.hermes/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other NAZAR_HOME appends a short hash for uniqueness.
    """
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hermes/nazar-agent  -> /home/alice/.hermes/nazar-agent
...
Environment="NAZAR_HOME={hermes_home}"
Restart=always
RestartSec=5
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only NAZAR_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp NAZAR_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="NAZAR_HOME=..."``. Writing that to the developer's
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        "  The image entrypoint normally drops privileges to the 'nazar' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Hermes command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$NAZAR_HOME and break later non-root dashboard/gateway runs."
    )
...
def _setup_whatsapp():
    ...
    from nazar_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from nazar_cli.dingtalk_auth import dingtalk_qr_auth
...
def _builtin_setup_fn(key: str):
    ...
    """
    from nazar_cli import setup as _s
    return {
...
def _dispatch_via_service_manager_if_s6(
    ...
    """
    from nazar_cli.service_manager import (
    ...
    if profile is None:
        ...
        # NAZAR_HOME=<root>/profiles/<name>, "" for the default root,
