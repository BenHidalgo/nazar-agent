...
Diagnoses issues with Nazar Agent setup.
"""
...
from nazar_cli.models import _HERMES_USER_AGENT
from nazar_cli.vercel_auth import describe_vercel_auth
from nazar_constants import OPENROUTER_MODELS_URL
...
from nazar_constants import is_termux as _is_termux
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from nazar_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from nazar_cli.auth import get_minimax_oauth_auth_status
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from nazar_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    config_path = NAZAR_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from nazar_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from nazar_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from nazar_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from nazar_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from nazar_cli.config import check_config_version, migrate_config
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "nazar"
    ...
    try:
        from nazar_constants import is_container as _is_container
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = NAZAR_HOME / "config.yaml"
    ...
    try:
        from nazar_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
