...
Each profile is a fully independent NAZAR_HOME directory with its own
...
_RESERVED_NAMES = frozenset({
    "nazar", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current NAZAR_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NAZAR_HOME points outside
    ``~/.hermes``, profiles live under ``NAZAR_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) NAZAR_HOME path.

    In standard deployments this is ``~/.hermes``.
    In Docker/custom deployments where NAZAR_HOME is outside ``~/.hermes``
    (e.g. ``/opt/data``), returns NAZAR_HOME directly.
    """
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NAZAR_HOME directory."""
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NAZAR_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NAZAR_HOME": str(profile_dir)},
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from nazar_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NAZAR_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NAZAR_HOME")
    try:
        os.environ["NAZAR_HOME"] = str(profile_dir)
        from nazar_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NAZAR_HOME"] = old_home
        elif "NAZAR_HOME" in os.environ:
            del os.environ["NAZAR_HOME"]
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NAZAR_HOME.

    Returns ``"default"`` if NAZAR_HOME is not set or points to ``~/.hermes``.
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NAZAR_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the NAZAR_HOME environment variable.
    """
