...
Shows the status of all Nazar Agent components.
"""
...
def show_status(args):
    """Show status of all Nazar Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Nazar Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if _effective_provider_label() == "LM Studio":
        from nazar_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from nazar_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
