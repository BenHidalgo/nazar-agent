...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Nazar Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"nazar" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "nazar" in their tag but are fully tool-capable.
#
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nazar_cli.model_switch import
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'nazar model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nazar doctor' — config issues detected:"
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from nazar_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nazar_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://nazar-agent.local/docs/api/model-catalog.json so
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from nazar_cli.auth import _load_auth_store
    ...
    from nazar_cli.providers import HERMES_OVERLAYS
    from nazar_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    # with `nazar model`).
    try:
        from nazar_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
...
def list_picker_providers(
    ...
    """
    from nazar_cli.models import fetch_openrouter_models
