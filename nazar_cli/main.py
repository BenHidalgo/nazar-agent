...
Usage:
    ...
    hermes uninstall           Uninstall Nazar Agent
...
# IMPORTANT: nazar_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``nazar_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``nazar update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``nazar_bootstrap`` but the editable install's
...
# ``nazar update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _print_fast_version_info() -> None:
    from nazar_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Nazar Agent v{__version__} ({__release_date__})")
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'nazar {command_name}' requires an interactive terminal.\n"
...
#
# Many modules cache NAZAR_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("NAZAR_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.hermes/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set NAZAR_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 1.5 If NAZAR_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.hermes/profiles/coder or
    # /opt/data/profiles/coder).  If NAZAR_HOME points to the hermes root
    # instead (e.g. systemd hardcodes NAZAR_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("NAZAR_HOME", "")
    ...
    # 3. If we found a profile, resolve and set NAZAR_HOME
    if profile_name is not None:
        try:
            from nazar_cli.profiles import resolve_profile_env
            ...
            return
        os.environ["NAZAR_HOME"] = hermes_home
...
try:
    ...
    _cfg_path = get_nazar_home() / "config.yaml"
    if _cfg_path.exists():
...
try:
    from nazar_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from nazar_constants import apply_ipv4_preference as _apply_ipv4
...
from nazar_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_nazar_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from nazar_cli.config import get_env_path, get_nazar_home, load_config
    from nazar_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from nazar_cli.config import DEFAULT_CONFIG
    ...
    from nazar_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_nazar_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'nazar')
    """
...
def _ensure_tui_node() -> None:
    ...
    hermes_home = os.environ.get("NAZAR_HOME") or str(Path.home() / ".hermes")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "NAZAR_HOME": hermes_home},
            capture_output=True,
            text=True,
            check=False,
        )
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from nazar_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from nazar_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `nazar update` so
    ...
    if code == 42:
        from nazar_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from nazar_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'nazar sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from nazar_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from nazar_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'nazar doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'nazar setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from nazar_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from nazar_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from nazar_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from nazar_cli.config import get_env_value, save_env_value
    ...
    session_dir = get_nazar_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Nazar Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
        ...
        print("⚠ Pairing may not have completed. Run 'nazar whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from nazar_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from nazar_cli.config import stamp_install_method
    from nazar_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``nazar model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from nazar_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from nazar_cli.providers import resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'nazar model' for "
                "available providers, or run 'nazar doctor' to diagnose config "
                "issues."
            )
    ...
    from nazar_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _clear_stale_openai_base_url():
    ...
    """
    from nazar_cli.config import get_env_value, save_env_value, load_config
...
# `nazar model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `nazar model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    try:
        from nazar_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from nazar_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``nazar model`` flow, then route aux tasks to them here.
    """
    from nazar_cli.config import load_config
    from nazar_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from nazar_cli.auth import _prompt_model_selection
    from nazar_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from nazar_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from nazar_constants import OPENROUTER_BASE_URL
    from nazar_cli.auth import (
        ProviderConfig,
    ...
    from nazar_cli.config import get_env_value
    ...
    # bad paste short of re-running `nazar setup` from scratch. OpenRouter
    ...
    from nazar_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from nazar_constants import AI_GATEWAY_BASE_URL
    from nazar_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from nazar_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from nazar_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from nazar_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from nazar_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from nazar_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from nazar_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            try:
                ...
                # can't reach the manual-paste path via ``nazar model``.
                mock_args = argparse.Namespace(
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from nazar_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from nazar_cli.auth import _save_model_choice, deactivate_provider
    from nazar_cli.config import get_env_value, load_config, save_config
    from nazar_cli.secret_prompt import masked_secret_prompt
    ...
    from nazar_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `nazar model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from nazar_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from nazar_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from nazar_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from nazar_cli import azure_detect
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from nazar_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from nazar_cli.config import save_env_value, load_config, save_config
    from nazar_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from nazar_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from nazar_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from nazar_cli.env_loader import format_secret_source_suffix
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``nazar setup`` / ``nazar model``.
    ...
    """
    from nazar_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from nazar_cli.config import save_env_value
    from nazar_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `nazar setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from nazar_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from nazar_cli.auth import AuthError
        from nazar_cli.models import fetch_lmstudio_models
        ...
        from nazar_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from nazar_constants import display_hermes_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from nazar_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'nazar model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    ...
    from nazar_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from nazar_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from nazar_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from nazar_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from nazar_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from nazar_cli.webhook import webhook_command
...
def cmd_portal(args):
    ...
    from nazar_cli.portal_cli import portal_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from nazar_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from nazar_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from nazar_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from nazar_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from nazar_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from nazar_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from nazar_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from nazar_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from nazar_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from nazar_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from nazar_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from nazar_cli.banner import check_for_updates
        from nazar_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Nazar Agent."""
    _require_tty("uninstall")
    from nazar_cli.uninstall import run_uninstall
...
# even run ``nazar update`` again to roll forward. The post-pull syntax
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``nazar update`` to stash the
    lockfile — repeatedly.
    """
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    forgotten.  When ``nazar update`` replaces files on disk, the running
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `nazar update`.
    ...
    print(
        "  Docs:         https://nazar-agent.local/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``nazar update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``nazar update`` invocations skip the block until a newer
...
def _kill_stale_dashboard_processes(
    ...
    Called at the end of ``nazar update`` (default ``reason``) and also
...
def _update_via_zip(args):
    """Update Nazar Agent by downloading a ZIP archive.
...
# =========================================================================
# Fork detection and upstream management for `nazar update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_nazar_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_nazar_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``nazar update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``nazar update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    so during ``nazar update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``nazar update`` invocation never
    ...
    # walking the parent chain, every ``nazar update`` reports its own
...
def _quarantine_running_hermes_exe(
    ...
    when ``nazar update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            ...
            "gateway, or pause AV scanning, then re-run `nazar update`."
        )
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``nazar update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `nazar update` once the upstream issue is resolved.")
...
class _UpdateOutputStream:
    """Stream wrapper used during ``nazar update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``nazar update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``nazar update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # hermes_cli.config.get_nazar_home to simulate setup failure.
        from nazar_cli.config import get_nazar_home as _get_nazar_home

        logs_dir = _get_nazar_home() / "logs"
...
def _cmd_update_check():
    ...
    from nazar_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from nazar_cli.config import recommended_update_command
        from nazar_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``nazar update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# Nazar Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of NAZAR_HOME before running the update.
    ...
    NAZAR_HOME directories.  The ``--backup`` flag on ``nazar update``
    ...
    try:
        from nazar_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
...
def cmd_update(args):
    """Update Nazar Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from nazar_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Nazar Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # every user who ran ``nazar update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``nazar update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_nazar_home added to hermes_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # seed_profile_skills() uses subprocess with an explicit NAZAR_HOME so
        # it is not affected by sync_skills()'s module-level NAZAR_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's NAZAR_HOME env var points at the default or a named profile.
        try:
            from nazar_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                print()
                print("Skipped. Run 'nazar config migrate' later to configure.")
        else:
        ...
        # Tying the refresh to ``nazar update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from nazar_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_nazar_home() / ".update_exit_code"
            try:
        ...
        # every `nazar update` surfaces the issue until the user migrates.
        try:
            from nazar_cli.gateway import (
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from nazar_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from nazar_constants import get_nazar_home as _hh
        ...
        from nazar_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `nazar profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from nazar_cli.profiles import profile_exists
        ...
        from nazar_cli.profiles import rename_profile
        ...
        from nazar_cli.profiles import export_profile
        ...
        from nazar_cli.profiles import import_profile
        ...
        import tempfile
        from nazar_cli.profile_distribution import (
            plan_install,
        ...
        from nazar_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from nazar_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from nazar_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No hermes dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `nazar update`.
    ...
    from nazar_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from nazar_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from nazar_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from nazar_cli.models import CANONICAL_PROVIDERS as _cp
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from nazar_cli.plugins import discover_plugins
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from nazar_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from nazar_cli.oneshot import run_oneshot
...
def main():
    ...
    try:
        from nazar_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``hermes.exe.old.*`` quarantine files left by previous
    # ``nazar update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # =========================================================================
    from nazar_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nazar-agent.local/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `nazar model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nazar-agent.local/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from nazar_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from nazar_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Nazar Agent with an interactive wizard. "
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'nazar setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_manifest.add_argument(
        ...
        "writes to $NAZAR_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Nazar")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from nazar_cli.send_cmd import register_send_subparser
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Nazar Agent components",
    )
    ...
    # =========================================================================
    from nazar_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from nazar_cli.kanban import build_parser as _build_kanban_parser
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Nazar Agent setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Nazar Agent. Use 'nazar debug share' to "
...
""",
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'nazar debug share'",
    )
    ...
    from nazar_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Nazar Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from nazar_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "so future 'nazar update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from nazar_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from nazar_cli.skills_hub import skills_command
    ...
    from nazar_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    def cmd_plugins(args):
        from nazar_cli.plugins_cmd import plugins_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from nazar_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from nazar_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_nazar_home() / "memories"
            ...
            from nazar_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'nazar tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from nazar_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from nazar_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "post-setup hook that `nazar tools` runs when you first\n"
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'nazar mcp add' to connect to a new server, or\n"
            "'nazar mcp serve' to expose Hermes conversations over MCP."
        ),
    )
    ...
    def cmd_mcp(args):
        from nazar_cli.mcp_config import mcp_command
    ...
    claw_migrate.add_argument(
        ...
        "before apply; restorable with 'nazar import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from nazar_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Nazar Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'nazar config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--force",
        action="store_true",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Nazar Agent",
        description="Remove Nazar Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Nazar Agent as an ACP (Agent Client Protocol) server",
        description="Start Nazar Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    def cmd_acp(args):
        """Launch Nazar Agent as an ACP server."""
        try:
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `nazar update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Nazar Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # path `nazar update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
...
""",
    ...
    from nazar_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'nazar -c model' where 'model' is consumed as the
