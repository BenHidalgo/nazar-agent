...
Subcommands:
  ...
  hermes fallback add      Pick provider + model via the same picker as `nazar model`,
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from nazar_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `nazar model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    print("  Docs: https://nazar-agent.local/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `nazar model`, then append the selection to the chain."""
    from nazar_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `nazar model` — select the provider + model you want as a fallback.")
    print()
