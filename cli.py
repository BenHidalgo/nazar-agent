...
# IMPORTANT: nazar_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See nazar_bootstrap.py for full rationale.
try:
...
from nazar_cli.fallback_config import get_fallback_chain
...
try:
    from nazar_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from nazar_cli.banner import _format_context_length, format_banner_version_label
...
from nazar_cli.browser_connect import (
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from nazar_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    """
    # Check user config first ({NAZAR_HOME}/config.yaml)
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from nazar_cli.config import _normalize_root_model_keys
    ...
    from nazar_cli.config import _expand_env_vars
...
try:
    from nazar_logging import setup_logging
...
try:
    from nazar_cli.config import print_config_warnings
...
try:
    from nazar_cli.skin_engine import init_skin_from_config
...
from nazar_cli.banner import build_welcome_banner
from nazar_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from nazar_cli.callbacks import prompt_for_secret
...
def _run_cleanup():
    ...
    try:
        from nazar_cli.plugins import invoke_hook as _invoke_hook
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    if session_db is None:
        return
    try:
        from nazar_cli.config import load_config as _load_full_config
        from nazar_constants import get_nazar_home as _get_nazar_home
        _hermes_home_maint = _get_nazar_home()
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from nazar_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from nazar_cli.skin_engine import get_active_skin
...
from nazar_constants import is_termux as _is_termux_environment
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Nazar Agent") if _skin else "Nazar Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from nazar_cli import __release_date__ as _release_date
        from nazar_cli import __version__ as _version

        version_line = f"Nazar Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from nazar_cli.plugins import get_plugin_commands
...
class HermesCLI:
    """
    Interactive CLI for the Nazar Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from nazar_cli.runtime_provider import _auto_detect_local_model
        ...
        # it's shared across all Hermes processes for this NAZAR_HOME.
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from nazar_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from nazar_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from nazar_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from nazar_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from nazar_cli.runtime_provider import (
        ...
        if not self.model and resolved_provider:
            try:
                from nazar_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from nazar_cli.models import resolve_fast_mode_overrides
    ...
    def _show_security_advisories(self):
        ...
        """
        try:
            from nazar_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        from nazar_cli.model_switch import is_nous_hermes_non_agentic
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with Nazar Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        """
        from nazar_cli.clipboard import save_clipboard_image

        img_dir = get_nazar_home() / "images"
    ...
    def _handle_paste_command(self):
        ...
        from nazar_cli.clipboard import has_clipboard_image
    ...
    def _show_session_status(self):
        ...
        # 2.1.114's /recap.
        try:
            from nazar_cli.session_recap import build_recap
            recap = build_recap(
    ...
    def _fast_command_available(self) -> bool:
        try:
            from nazar_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from nazar_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from nazar_cli.skin_engine import get_active_help_header
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        from nazar_cli.tools_config import _get_platform_tools
        from nazar_cli.config import load_config
    ...
    def _handle_profile_command(self):
        ...
        from nazar_constants import display_hermes_home
        from nazar_cli.profiles import get_active_profile_name
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from nazar_cli.main import _relative_time
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from nazar_state import format_session_db_unavailable
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if target.isdigit():
            ...
            from nazar_cli.main import _resolve_session_by_name_or_id
    ...
    def save_conversation(self):
        ...
        DB, so the live session remains resumable via ``nazar --resume <id>``
        ...
        saved_dir = get_nazar_home() / "sessions" / "saved"
        try:
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: nazar --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from nazar_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from nazar_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            if not model_list:
                try:
                    from nazar_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from nazar_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from nazar_cli.model_switch import switch_model, parse_model_flags
        from nazar_cli.providers import get_label
        ...
        # clobber disk config.
        from nazar_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        """
        from nazar_cli import codex_runtime_switch as crs
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from nazar_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        try:
            from nazar_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from nazar_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        from nazar_cli.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        from nazar_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from nazar_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from nazar_cli.skin_engine import get_active_skin
            ...
            from nazar_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from nazar_cli.plugins import get_plugin_manager
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from nazar_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from nazar_cli.goals import GoalManager
            from nazar_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from nazar_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from nazar_cli.config import load_config
        from nazar_cli.colors import Colors as _Colors
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from nazar_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from nazar_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        ...
        when cancelled.
        """
        from nazar_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Nazar Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Nazar Agent"),
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from nazar_cli.config import get_config_path as _get_config_path
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "nazar_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "nazar_voice",
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard NAZAR_HOME, so no -p needed.
            try:
                from nazar_cli.profiles import get_active_profile_name
            ...
            print(f"  nazar --resume {self.session_id}{profile_flag}")
            if session_title:
            ...
            try:
                from nazar_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from nazar_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from nazar_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Nazar Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Nazar Agent! Type your message or /help for commands."
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from nazar_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from nazar_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Nazar Agent")
