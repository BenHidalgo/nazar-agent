...
try:
    ...
    def get_nazar_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("NAZAR_HOME") or "").strip()
...
DEFAULT_LOG = Path(os.environ.get("HERMES_PERF_LOG", str(get_nazar_home() / "perf.log")))
DEFAULT_STATE_DB = get_nazar_home() / "state.db"
