"""Shared constants for Nazar Agent.
...
_UNSET = object()
_NAZAR_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_NAZAR_HOME_OVERRIDE", default=_UNSET
)
...
def get_nazar_home() -> Path:
    """Return the Hermes home directory (default: ~/.nazzar).

    Reads NAZAR_HOME env var, falls back to ~/.nazzar.
    This is the single source of truth — all other copies should import this.

    When ``NAZAR_HOME`` is unset but an ``active_profile`` file indicates
    a non-default profile is active, logs a loud one-shot warning to
    ``errors.log`` so cross-profile data corruption is diagnosable instead
    of silent.  Behavior is unchanged otherwise — we still return
    ``~/.nazzar`` — because raising here would brick 30+ module-level
    callers that import this at load time.  Subprocess spawners are
    expected to propagate ``NAZAR_HOME`` explicitly (see the systemd
    ...
    val = os.environ.get("NAZAR_HOME", "").strip()
    if val:
        return Path(val)
    ...
    if not _profile_fallback_warned:
        try:
            ...
            active_path = (Path.home() / ".nazzar" / "active_profile")
        ...
        if active and active != "default":
            ...
            import sys
            msg = (
                f"[NAZAR_HOME fallback] NAZAR_HOME is unset but active "
                f"profile is {active!r}. Falling back to ~/.nazzar, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass NAZAR_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return Path.home() / ".nazzar"


def get_default_hermes_root() -> Path:
    """Return the root Hermes directory for profile-level operations.

    In standard deployments this is ``~/.nazzar``.

    In Docker or custom deployments where ``NAZAR_HOME`` points outside
    ``~/.nazzar`` (e.g. ``/opt/data``), returns ``NAZAR_HOME`` directly
    — that IS the root.

    In profile mode where ``NAZAR_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.nazzar/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = Path.home() / ".nazzar"
    env_home = os.environ.get("NAZAR_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # NAZAR_HOME is under ~/.nazzar (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — NAZAR_HOME itself is the root
    return env_path
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<NAZAR_HOME>/skills`` last-resort
    """
...
def get_nazar_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to NAZAR_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to NAZAR_HOME (e.g. ``"image_cache"``).
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current NAZAR_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.nazzar``
        profile:  ``~/.nazzar/profiles/coder``
        custom:   ``/opt/hermes-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.nazzar``.  For code that needs a real ``Path``, use
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``NAZAR_HOME`` or other path
...
def get_subprocess_home() -> str | None:
    ...
    When ``{NAZAR_HOME}/home/`` exists on disk, subprocesses should use it
