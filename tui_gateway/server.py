...
def resolve_skin() -> dict:
    try:
        from nazar_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from nazar_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from nazar_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from nazar_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from nazar_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from nazar_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from nazar_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from nazar_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from nazar_cli.config import load_config as _load_full_cfg
...
#
# Layout:  $NAZAR_HOME/spawn-trees/<session_id>/<timestamp>.json
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from nazar_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from nazar_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from nazar_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from nazar_cli.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    try:
        from nazar_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nazar_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from nazar_cli.config import is_managed, save_env_value
        from nazar_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from nazar_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from nazar_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from nazar_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from nazar_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from nazar_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from nazar_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from nazar_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from nazar_cli.tools_config import (
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from nazar_cli.skills_hub import do_install
        ...
        if action == "browse":
            from nazar_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from nazar_cli.skills_hub import inspect_skill
