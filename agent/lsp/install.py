...
``<NAZAR_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
# ``<NAZAR_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def hermes_lsp_bin_dir() -> Path:
    ...
    home = os.environ.get("NAZAR_HOME")
    if home is None:
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <NAZAR_HOME>/lsp/
