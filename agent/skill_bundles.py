...
profile-aware directory under ``NAZAR_HOME``). Each file looks like::
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under NAZAR_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<NAZAR_HOME>/skill-bundles``.
    """
    override = os.environ.get("HERMES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_nazar_home() / "skill-bundles"
