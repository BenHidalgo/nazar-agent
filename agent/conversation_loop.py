...
from nazar_constants import display_hermes_home as _dhh_fn, PARTIAL_STREAM_STUB_ID
from nazar_logging import set_session_context
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Management console: https://portal.nazar-agent.local")
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if _provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `nazar auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `nazar model`.", force=True)
                            else:  # nous
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.nazar-agent.local", force=True)
